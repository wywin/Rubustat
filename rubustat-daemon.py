...
while 1 == 1:
    ...
    indoor_temp = float(subprocess.check_output("./etc.sh indoor_temp", shell=True))
    outdoor_temp = float(subprocess.check_output("./etc.sh outdoor_temp", shell=True))
    hvac_state = int(subprocess.check_output("./etc.sh get_state", shell=True))
    ...
    if outdoor_temp < set_temp:
        if hvac_state == 0: #idle
            if indoor_temp < set_temp - inactive_hysteresis:
                os.system("./etc.sh heat", shell=True)
                hvac_state = 1
        elif hvac_state == 1: #heating
            if indoor_temp > set_temp + active_hysteresis:
                os.system("./etc.sh fan_to_idle", shell=True) 
                time.sleep(300)
                os.system("./etc.sh idle", shell=True)
                hvac_state = 0
    ...
    # ac mode
        ...
        if hvac_state == 0: #idle
            if indoor_temp > set_temp + inactive_hysteresis:
                os.system("./etc.sh cool", shell=True)
                hvac_state = -1
